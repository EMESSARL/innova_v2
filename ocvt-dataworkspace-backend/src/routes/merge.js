...
router.post(
  ...
  [
    check("datasets")
      .isArray()
      .withMessage("datasets doit être un tableau")
      .notEmpty()
      .withMessage("datasets ne peut pas être vide")
      .custom((value, { req }) => {
    ...
    check("key_mappings")
      ...
      .custom((value, { req }) => {
        const datasetIds = req.body.datasets.map((ds) =>
          ds.dataset_id.toString()
        );
        for (const key in value) {
          if (!datasetIds.includes(key)) {
            throw new Error(
              `key_mappings contient une clé (${key}) qui ne correspond à aucun dataset_id fourni`
            );
          }
          if (typeof value[key] !== "string" || value[key].trim() === "") {
            throw new Error(
              `La valeur de key_mappings pour le dataset ${key} doit être une chaîne non vide`
            );
          }
        }
        if (Object.keys(value).length !== datasetIds.length) {
          throw new Error(
            "key_mappings doit contenir une entrée pour chaque dataset_id fourni"
          );
        }
        return true;
      }),
    ...
    check("suffixes")
      ...
      .custom((value, { req }) => {
        if (value && value.length < req.body.datasets.length) {
          throw new Error(
